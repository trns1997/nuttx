...
if [ -d "${ARCHDIR}" ]; then
    ...
    set -- "${ARCHDIR}"/*.h
    if [ -e "$1" ]; then
        echo "Copying architecture headers from ${ARCHDIR} to ${EXPORTDIR}/arch"
    ...
    echo "Warning: Architecture directory ${ARCHDIR} does not exist" >&2
fi
...
if [ "X${USRONLY}" != "Xy" ]; then
  ...
  for hdir in $ARCH_HDRDIRS; do
    ...
    if [ -d "$srcdir" ] || [ -h "$srcdir" ]; then
      mkdir -p "$dstdir" || {
        echo "Error: mkdir $dstdir failed" >&2
        exit 1
      }
      ...
      set -- "$srcdir"/*.h
      if [ -e "$1" ]; then
        echo "Copying headers from $srcdir to $dstdir"
        if ! cp -f "$@" "$dstdir/"; then
          echo "Error: Failed to copy headers from $srcdir" >&2
          exit 1
        fi
      else
        echo "Notice: No header files found in $srcdir" >&2
      fi

      # Handle hardware subdir if it exists
      if [ -d "$srcdir/hardware" ]; then
        mkdir -p "$dstdir/hardware" || {
          echo "Error: mkdir $dstdir/hardware failed" >&2
          exit 1
        }

        set -- "$srcdir/hardware"/*.h
        if [ -e "$1" ]; then
          echo "Copying headers from $srcdir/hardware to $dstdir/hardware"
          if ! cp -f "$@" "$dstdir/hardware/"; then
            echo "Error: Failed to copy headers from $srcdir/hardware" >&2
            exit 1
          fi
        else
          echo "Notice: No hardware headers in $srcdir/hardware" >&2
        fi
      fi
    fi
  done
  ...
  mkdir -p "${EXPORTDIR}/arch/os" || {
    echo "Error: mkdir ${EXPORTDIR}/arch/os failed" >&2
    exit 1
  }
  ...
  for dir in ${OSDIRS}; do
    ...
    set -- "$srcdir"/*.h
    if [ -e "$1" ]; then
      echo "Copying OS headers from $srcdir to $dstdir"
      if ! cp -f "$@" "$dstdir/"; then
        echo "Error: Failed to copy headers from $srcdir" >&2
        exit 1
      fi
    else
      echo "Notice: No headers in $srcdir" >&2
    fi
  done
